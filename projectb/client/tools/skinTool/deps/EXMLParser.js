...
//新增加的属性
/*var propTypeList = {number:["tag","anthorPerX","anthorPerY","maskX","maskY","imgX","imgY","imgScaleX","imgScaleY",
							"paddingBottom","paddingTop","paddingRight","paddingLeft","anitp","gap","width","height","x","y",
							"maxChars","textColor","tableSize","size","promptColor","fontSize","throwSpeed","verticalCenter",
							"horizontalCenter","left","right","top","bottom","alpha","stroke","strokeColor","percentHeight","percentWidth",
							"lineSpacing","value","letterSpacing","maximum","itemW","itemH",
							"horizontalGap","verticalGap"],
					string:["source","mSource","cSource","stateStr","currState","scrollPolicyV","scrollPolicyH","sound","skName","aniName","skinName",
							"textAlign","fontFamily","prompt","fileName","text","verticalAlign","icon","label","font","effectId",
							"title","direction"],
					boolean:["keepSelect","activeCheckEnable","activeInView","canOutBound","autoPlay","enableAniCache","isHorizontal","bold","bounces","visible","repeatClk","showItemInfoOnClick","hideLabelText","multiline","selected","performanceControl","enabled","italic"],
					any:["viewport","layout","itemRenderer"],
					Array:["elementsContent"],
					Rectangle:["scale9Grid"]};
					
//新增加的UI组件 对应eui组件表
var ctrl2eui = {Image:"Image", MaskImage:"Component", MenuGroup:"Group", Component:"Component",Group:"Group", TableScroller:"Group",
				...
				Tab_Sys:"Button",UIEffect:"Image",			
				EditableText:"EditableText"
				};
						
var defaultValList = {x:0,y:0,width:0,height:0,anchorOffsetX:0,anchorOffsetY:0};*/


//修改记录
//eui中的   EXState  EXSetProperty  

var ctrl2id = [];


				
var exmlConfig = new sys.EXMLConfig();

function getPropertyType( prop, className, tp )
{
	tp = tp || 0;
	//console.log( "tp=" + tp + "  prop=" + prop + "  className=" + className );
	var ret = null;
	if( !className || !prop ) return ret;
	
	var propTypeList = EXMLParser.conf.propTypeList;
	...
	if( !ret )
	{
		var ctrl2eui = EXMLParser.conf.ctrl2eui;
		var euiCtrlName = ctrl2eui[className];
		if( !euiCtrlName ) {
			var idx = className.indexOf( "." );
			if( idx>0 ){
				euiCtrlName = ctrl2eui[className.substring(idx+1)];
			}
		}
...
function getDefaultPropById( localName, namespace )
{
	var euiCtrlName = EXMLParser.conf.ctrl2eui[localName];
	if( euiCtrlName )
	{
		localName = euiCtrlName;
		namespace = "http://ns.egret.com/eui";
	}
	
	return exmlConfig.getDefaultPropById(localName, namespace);
}
...
var EXMLParser = (function () {
	...
	EXMLParser.prototype.rmvExcludeCtrl = function(xmlData){
		...
		if (children) {
			for (var i = children.length-1; i >=0; i--) {
				...
				if( id && id.indexOf("rmv_")>=0 ){
					console.log( "rmv node id=" + id );
					children.splice(i,1);
				}
			}
		}
	...
	EXMLParser.prototype.addNeedRes = function ( resName ) {
		var idx = resName.indexOf( "@" );
		if( idx>0 ){
			resName = resName.substring( 0, idx ) + "#st";
		}
		
		if( this.needRess.indexOf(resName)<0 ){
			this.needRess.push( resName );
		}
	}
	/**
	 * @private
	...
	EXMLParser.prototype.addIds = function (items) {
		...
		for (var i = 0; i < length; i++) {
			...
			var id = node.attributes.id;
			if( id ){
				if( id=='iconDisplay' ) id = 'skIcon';
				else if( id=='labelDisplay' ) id = 'skLabel';
		...
		if (!result) {
			if (this.isProperty(node)) {
				...
				if (this.isProperty(parent_1)) {
					...
					prop = getDefaultPropById(parent_1.localName, parent_1.namespace);
				}
				var className = this.getClassNameOfNode(parent_1);
				result = (getPropertyType(prop, className, 1) == TYPE_CLASS);
			}
		}
	...
	EXMLParser.prototype.getNodeId = function (node) {
		...
		var idx = ctrl2id.indexOf(node.localName);
		if( idx < 0 ){
			idx = ctrl2id.length;
			ctrl2id.push(node.localName);
		}
	...
	EXMLParser.prototype.addAttributesToCodeBlock = function (cb, varName, node) {
		...
		if( attributes.txtKey ){
			var keyIdx = keyList.indexOf( "text" );
			if(keyIdx>=0) keyList.splice(keyIdx,1); 

			keyIdx = keyList.indexOf( "label" );
			if(keyIdx>=0) keyList.splice(keyIdx,1);
		}

		if( attributes.txtFlowKey ){
			var keyIdx = keyList.indexOf( "textFlow" );
			if(keyIdx>=0) keyList.splice(keyIdx,1); 
		}

		if( attributes.source ){
			this.addNeedRes( attributes.source );
		}
		else if( attributes.font ){
			this.addNeedRes( attributes.font + "#fnt" );
		}
		else if( attributes.icon ){
			this.addNeedRes( attributes.icon );
		}
	...
	EXMLParser.prototype.initlizeChildNode = function (node, cb, varName) {
		...
		for (var i = 0; i < length; i++) {
			...
			if (this.isInnerClass(child)) {
				if (child.localName == "Skin") {
					...
					var type = getPropertyType(SKIN_NAME, className, 2);
					if (type) {
			...
			if (this.isProperty(child)) {
				...
				var type = getPropertyType(child.localName, className, 3);
				if (!type) {
					if (true) {
		...
		if (directChild.length == 0)
			return;
		var defaultProp = getDefaultPropById(node.localName, node.namespace);
		
		var defaultType = getPropertyType(defaultProp, className, 4);
		if (true) {
	...
	EXMLParser.prototype.isProperty = function (node) {
		...
		if (!name || node.nodeType !== 1 || !node.parent || this.isBasicTypeData(name)) {
			...
			result = !!getPropertyType(name, className, 5);
		}
	...
	EXMLParser.prototype.formatKey = function (key, value) {
		...
		else if( key == "verticalCenter" ){
			key = "vCenter";
		}
		else if( key == "horizontalCenter" ){
			key = "hCenter";
		}
		return key;
	};
	...
	EXMLParser.prototype.formatValue = function (key, value, node) {
		...
		value = value.trim();
		var className = this.getClassNameOfNode(node);
		var type = getPropertyType(key, className, 6);
		if (true && !type) {
			egret.log(2005, this.currentClassName, key, toXMLString(node));
		}
		...
		if (bindingValue) {
			...
			value = "new egret." + RECTANGLE + "(" + value + ")";
		}
			...
			switch (type) {
				...
				case "number":
					...
					else if( EXMLParser.conf.defaultValList.hasOwnProperty(key) ){
						var tmpInt = Math.floor(parseFloat(value)+0.5);
					
						value = EXMLParser.conf.defaultValList[key]!=tmpInt ? tmpInt.toString() : null;
					}
				...
				default:
					if (true) {
						egret.log(2008, this.currentClassName, "string", key + ":" + type, toXMLString(node));
					}
					break;
			}
		}
	...
	EXMLParser.prototype.createConstructFunc = function () {
		...
		if (length > 0) {
			...
			this.currentClass.addStaticVar( new sys.EXVariable("skinParts",skinPartStr)  );
		}
		...
		var needRess = this.needRess;
		length = needRess.length;
		if (length > 0) {
			var needRessStr = "[]";
			for (var i = 0; i < length; i++) {
				needRess[i] = "\"" + needRess[i] + "\"";
			}
			needRessStr = "[" + needRess.join(",") + "]";
			this.currentClass.addStaticVar( new sys.EXVariable("needRess",needRessStr)  );
		}
	...
	EXMLParser.prototype.getStateNames = function () {
		...
		var type = getPropertyType("states", className, 7);
		if (type != TYPE_STATE) {
			return;
		}
	...
	EXMLParser.prototype.createStates = function (parentNode) {
		...
		for (var i = 0; i < length; i++) {
			...
			if (this.isProperty(node)) {
				...
				var type = getPropertyType(prop, className, 8);
				if (true) {
					if (type == TYPE_ARRAY) {
	...
	EXMLParser.prototype.getClassNameOfNode = function (node) {
		...
		//console.log( "getClassNameOfNode=" + className + " localName=" + node.localName + "  node.namespace=" + node.namespace  );
		if( !className ){
			if( node.namespace ){
				var idx = node.namespace.indexOf( "." );
				className = node.namespace.substring(0, idx+1) + node.localName;
			}
			// if( node.namespace=="game.*" )
			// {
			// 	className = "game." + node.localName;
			// }
			// else 
			// {
			// 	className = "cui." + node.localName;
			// }
		}
